...
export class MobileBuildsService {
  ...
  async ingestGithubBuild(workspaceId: string, mobileAppId: string, rawInput: unknown) {
    ...
    if (!repository) {
      throw new BadRequestException('Repository is not linked to this mobile application');
    }

    const status = this.normalizeStatus(input.status, input.conclusion);

    return this.prisma.mobileBuild.upsert({
      where: {
      ...
      create: {
        ...
        status,
        startedAt: this.optionalDate(input.startedAt),
        completedAt: this.optionalDate(input.completedAt),
        durationMs: this.optionalNumber(input.durationMs),
      },
      update: {
        commitSha,
        branch,
