...
export type MobileAnalysisEvidence = Record<string, unknown>;

export interface NormalizedMobileCrashTelemetry {
  [key: string]: unknown;
}

export interface NormalizedMobilePerformanceTelemetry {
  [key: string]: unknown;
}

export interface NormalizedMobileTelemetryVersion {
  [key: string]: unknown;
}

export interface MobileTelemetrySnapshot {
  provider?: 'FIREBASE' | 'SENTRY' | 'DATADOG' | 'NEW_RELIC' | 'CUSTOM';
  status?: string;
  externalProjectId?: string;
  crashes?: NormalizedMobileCrashTelemetry | null;
  performance?: NormalizedMobilePerformanceTelemetry | null;
  versions?: NormalizedMobileTelemetryVersion[];
  collectedAt?: string;
  [key: string]: unknown;
}
