...
export interface CreatedDevelopmentEntity<TPayload> {
  id: string;
  payload: TPayload;
...
export const developmentRoutes = {
  templates(workspaceId: string): string {
    return `/api/v1/workspaces/${workspaceId}/development/templates`;
  },

  root(workspaceId: string, applicationId: string): string {
    return `/api/v1/workspaces/${workspaceId}/applications/${applicationId}/development`;
  },

  summary(workspaceId: string, applicationId: string): string {
    return `${this.root(workspaceId, applicationId)}/summary`;
  },

  applyTemplate(workspaceId: string, applicationId: string): string {
    return `${this.root(workspaceId, applicationId)}/apply-template`;
  },

  milestones(workspaceId: string, applicationId: string): string {
    return `${this.root(workspaceId, applicationId)}/milestones`;
  },

  milestone(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.milestones(workspaceId, applicationId)}/${milestoneId}`;
  },

  completeMilestone(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.milestone(workspaceId, applicationId, milestoneId)}/complete`;
  },

  reopenMilestone(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.milestone(workspaceId, applicationId, milestoneId)}/reopen`;
  },

  skipMilestone(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.milestone(workspaceId, applicationId, milestoneId)}/skip`;
  },

  reorderMilestones(workspaceId: string, applicationId: string): string {
    return `${this.milestones(workspaceId, applicationId)}/reorder`;
  },

  tasks(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.milestone(workspaceId, applicationId, milestoneId)}/tasks`;
  },

  task(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.root(workspaceId, applicationId)}/tasks/${taskId}`;
  },

  taskStatus(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.task(workspaceId, applicationId, taskId)}/status`;
  },

  completeTask(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.task(workspaceId, applicationId, taskId)}/complete`;
  },

  reopenTask(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.task(workspaceId, applicationId, taskId)}/reopen`;
  },

  skipTask(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.task(workspaceId, applicationId, taskId)}/skip`;
  },

  moveTask(workspaceId: string, applicationId: string, taskId: string): string {
    return `${this.task(workspaceId, applicationId, taskId)}/move`;
  },

  reorderTasks(workspaceId: string, applicationId: string, milestoneId: string): string {
    return `${this.tasks(workspaceId, applicationId, milestoneId)}/reorder`;
  },

  blockers(workspaceId: string, applicationId: string): string {
    return `${this.root(workspaceId, applicationId)}/blockers`;
  },

  blocker(workspaceId: string, applicationId: string, blockerId: string): string {
    return `${this.blockers(workspaceId, applicationId)}/${blockerId}`;
  },

  resolveBlocker(workspaceId: string, applicationId: string, blockerId: string): string {
    return `${this.blocker(workspaceId, applicationId, blockerId)}/resolve`;
  },

  reopenBlocker(workspaceId: string, applicationId: string, blockerId: string): string {
    return `${this.blocker(workspaceId, applicationId, blockerId)}/reopen`;
  },
} as const;
...
export function expectDevelopmentSuccess(response: Response): void {
...
export function findNumberDeep(value: unknown, keys: string[]): number | undefined {
  ...
  for (const candidate of Object.values(record)) {
    const found = findNumberDeep(candidate, keys);
...
export async function applyDevelopmentTemplate(actor: WorkspaceTestUser, applicationId: string, template: DevelopmentTemplateType = enumValue(DevelopmentTemplateType), replaceExisting = false): Promise<Response> {
...
export async function createMilestone(actor: WorkspaceTestUser, applicationId: string, overrides: Partial<MilestonePayload> = {}): Promise<CreatedDevelopmentEntity<MilestonePayload>> {
  ...
  return {
    id: readEntityId(response, ['milestone']),

    payload,

    record: readDevelopmentRecord(response, ['milestone']),

    response,
  };
}

export async function updateMilestone(actor: WorkspaceTestUser, applicationId: string, milestoneId: string, payload: Partial<MilestonePayload>): Promise<Response> {
  return actor.agent
    .patch(developmentRoutes.milestone(actor.workspaceId, applicationId, milestoneId))
...
export async function skipMilestone(actor: WorkspaceTestUser, applicationId: string, milestoneId: string, reason = 'Removed from current scope'): Promise<Response> {
  return actor.agent
    .post(developmentRoutes.skipMilestone(actor.workspaceId, applicationId, milestoneId))
    .set(withBearer(actor.accessToken))
    .send({
      reason,
    });
...
export async function createTask(actor: WorkspaceTestUser, applicationId: string, milestoneId: string, overrides: Partial<TaskPayload> = {}): Promise<CreatedDevelopmentEntity<TaskPayload>> {
  const payload = buildTaskPayload(overrides);
  const response = await actor.agent
    .post(developmentRoutes.tasks(actor.workspaceId, applicationId, milestoneId))
    .set(withBearer(actor.accessToken))
    .send(payload);
  ...
  return {
    id: readEntityId(response, ['task']),

    payload,

    record: readDevelopmentRecord(response, ['task']),
...
export async function moveTask(actor: WorkspaceTestUser, applicationId: string, taskId: string, targetMilestoneId: string, position?: number): Promise<Response> {
  const payload: {
  ...
  return actor.agent
    .post(developmentRoutes.moveTask(actor.workspaceId, applicationId, taskId))
    .set(withBearer(actor.accessToken))
    .send(payload);
}
...
export async function createBlocker(actor: WorkspaceTestUser, applicationId: string, overrides: Partial<BlockerPayload> = {}): Promise<CreatedDevelopmentEntity<BlockerPayload>> {
  ...
  return {
    id: readEntityId(response, ['blocker']),

    payload,

    record: readDevelopmentRecord(response, ['blocker']),
...
export async function resolveBlocker(actor: WorkspaceTestUser, applicationId: string, blockerId: string, resolution = 'Credentials were issued'): Promise<Response> {
  return actor.agent
    .post(developmentRoutes.resolveBlocker(actor.workspaceId, applicationId, blockerId))
    .set(withBearer(actor.accessToken))
    .send({
      resolution,
    });
