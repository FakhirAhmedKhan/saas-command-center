...
- Name: batch10\auth.spec.ts >> Batch 10 authentication flows >> redirects the public root to login when the session is missing
- Location: e2e\batch10\auth.spec.ts:5:7
...
Locator: getByRole('heading', { name: 'Sign in to continue' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Sign in to continue' })
