...
- Name: phase16\releases-deployments.spec.ts >> Phase 16 releases and deployments >> shows an inline alert when a deployment transition fails
- Location: e2e\phase16\releases-deployments.spec.ts:475:7
...
Expected: "Rollback target must be a successful deployment."
Error: strict mode violation: getByRole('alert') resolved to 2 elements:
    1) <div role="alert" class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">No earlier successful deployment exists for this …</div> aka getByText('No earlier successful')
