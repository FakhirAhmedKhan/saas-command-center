import { APPLICATION_ID, installMockApi, makeWorkspace, PRIMARY_WORKSPACE_ID } from './fixtures/mock-api';
...
test.describe('Batch 10 frontend role authorization', () => {
  ...
  test('keeps lifecycle controls read-only for a viewer', async ({ page }) => {
    ...
    await expect(page.getByText('Only workspace owners and administrators can archive or restore applications.')).toBeVisible();
    await expect(page.getByText('Only the workspace owner can permanently delete an application.')).toBeVisible();
    await expect(
