...
export class SafeHttpClientService {
  ...
  async execute(input: SafeHealthRequestInput): Promise<SafeHealthRequestResult> {
    ...
    try {
      ...
      if (!selectedAddress) {
        throw new Error('No public IP address was resolved.');
      }
      ...
      const response = await request(url, {
        ...
        headers: {
          ...
          'user-agent': 'SaaS-Command-Center-Monitor/1.0',
      ...
      if (statusCode < input.expectedStatusMin || statusCode > input.expectedStatusMax) {
        return {
          ...
          failureReason: `Unexpected HTTP status ${statusCode}. Expected ${input.expectedStatusMin}-${input.expectedStatusMax}.`,
        };
      }

      if (responseTimeMs > input.degradedAfterMs) {
        return {
          ...
          failureReason: `Response time ${responseTimeMs}ms exceeded the degraded threshold of ${input.degradedAfterMs}ms.`,
        };
      }
      ...
      return {
        ...
        failureReason: this.normalizeError(error),
      };
    } finally {
      if (dispatcher) {
  ...
  private parseUrl(rawUrl: string): URL {
    ...
    try {
      ...
      throw new BadRequestException('Health-check URL is invalid.');
    }

    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
      throw new BadRequestException('Only HTTP and HTTPS health-check URLs are allowed.');
    }

    if (url.username || url.password) {
      throw new BadRequestException('Health-check URLs cannot contain embedded credentials.');
    }
  ...
  private async resolvePublicAddresses(hostname: string): Promise<ResolvedAddress[]> {
    ...
    try {
      ...
      throw new BadRequestException('Health-check hostname could not be resolved.');
    }

    if (addresses.length === 0) {
      throw new BadRequestException('Health-check hostname did not resolve to an IP address.');
    }
  ...
  private normalizeError(error: unknown): string {
    ...
    return 'Unknown monitoring failure.';
  }
}
