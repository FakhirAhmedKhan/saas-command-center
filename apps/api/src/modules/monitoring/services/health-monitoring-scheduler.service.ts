...
} from './health-check-runner.service';

@Injectable()
export class HealthMonitoringSchedulerService {
    private readonly logger =
        new Logger(
            HealthMonitoringSchedulerService.name,
        );
    ...
    async cleanupHistory():
        ...
        this.logger.log(
            `Deleted ${result.count} expired health-check history records.`,
        );
    }
    ...
    private async runWithLock(
        ...
        if (
            ...
            this.logger.debug(
                `Health check ${checkId} is already running on another instance.`,
            );
        }
    }
}