...
export class TokenService {
  ...
  async issueTokenPair(
    ...
    return {
      ...
      refreshExpiresAt: new Date(
        Date.now() +
        this.refreshExpiresInSeconds * 1000,
      ),
    };
  }
  ...
  private parseDuration(
    ...
    const match =
      /^(\d+)(s|m|h|d)$/.exec(normalized);

    if (!match) {
      throw new Error(
        `Invalid token duration "${value}". Use values such as 30s, 15m, 1h or 30d.`,
      );
    }
    ...
    if (unitMultiplier === undefined) {
      throw new Error(
        `Unsupported token duration unit "${unit}"`,
      );
    }
