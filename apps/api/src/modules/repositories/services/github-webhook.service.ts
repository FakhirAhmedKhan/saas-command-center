...
export class GithubWebhookService {
    ...
    private async processEvent(
        ...
        if (
            ...
            await this.repositoriesService.deleteInstallationEverywhere(
                externalInstallationId,
            );
        ...
        if (
            (event === 'installation_repositories' || event === 'repository') &&
            externalInstallationId
        ) {
            await this.repositoriesService.syncInstallationEverywhere(
                externalInstallationId,
            );
        ...
        if (
            ...
            await this.repositoriesService.touchRepositoryEverywhere(
                externalInstallationId,
                externalRepoId,
            );
        }
    ...
    private verifySignature(rawBody: Buffer, signature: string): void {
        const expected =
            `sha256=${createHmac('sha256', this.githubApp.getWebhookSecret())
                .update(rawBody)
                .digest('hex')}`;
