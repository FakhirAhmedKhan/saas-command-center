...
export class GithubAppService {
    ...
    async getInstallation(installationId: string): Promise<GithubInstallation> {
        const data = await this.appJson<GithubInstallationResponse>(
            `/app/installations/${this.numericId(installationId)}`,
            {
                method: 'GET',
            },
        );
    ...
    async exchangeUserCode(code: string, codeVerifier: string): Promise<string> {
        ...
        if (!response.ok || !data.access_token) {
            throw new BadGatewayException(
                data.error_description ?? data.error ?? 'GitHub user authorization failed.',
            );
        }
    ...
    async userCanAccessInstallation(
        ...
        const response = await fetch(
            `https://api.github.com/user/installations/${this.numericId(
                installationId,
        ...
        if (!response.ok) {
            throw new BadGatewayException(
                `GitHub installation authorization check failed with status ${response.status}.`,
            );
        }
    ...
    async listInstallationRepositories(installationId: string): Promise<GithubRepository[]> {
        ...
        for (let page = 1; ; page += 1) {
            const data = await this.tokenJson<GithubRepositoriesResponse>(
                token,
    ...
    private async createInstallationAccessToken(installationId: string): Promise<string> {
        const data = await this.appJson<GithubInstallationTokenResponse>(
            `/app/installations/${this.numericId(installationId)}/access_tokens`,
            {
                method: 'POST',
            },
        );
    ...
    private required(name: string): string {
        ...
        if (!value) {
            throw new ServiceUnavailableException(
                `GitHub integration is not configured: ${name} is missing.`,
            );
        }
