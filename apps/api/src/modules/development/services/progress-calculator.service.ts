...
export class ProgressCalculatorService {
    calculateSnapshot(
    ...
    ): ApplicationProgressExplanation {
        const milestoneResults =
            milestones.map((milestone) =>
                this.calculateMilestone(milestone),
            );

        const included = milestoneResults.filter(
            (milestone) => milestone.included,
        );

        const totalMilestoneWeight = included.reduce(
            (total, milestone) =>
                total + milestone.weight,
            0,
        );

        const weightedProgressPoints =
            included.reduce(
                (total, milestone) =>
                    total +
                    milestone.weight *
        ...
        return {
            ...
            excludedMilestones:
                milestoneResults.length - included.length,
            formula:
                'sum(milestone progress × milestone weight) ÷ sum(applicable milestone weights)',
    ...
    async recalculateWithTransaction(
        ...
        const application =
            await transaction.saasApplication.findFirst({
                where: {
                    id: applicationId,
                    workspaceId,
                },
        ...
        const explanation =
            this.calculateSnapshot(milestones);
    ...
    private calculateMilestone(
        ...
        const applicableTaskWeight =
            applicableTasks.reduce(
                (total, task) =>
                    total + task.weight,
                0,
            );

        const completedTaskWeight =
            applicableTasks
                .filter(
                    (task) =>
                        task.status ===
