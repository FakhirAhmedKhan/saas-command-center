...
export function calculateSessionMetrics(
    ...
    if (events.length === 0) {
        throw new Error(
            'Session metrics cannot be calculated without events',
        );
    }
    ...
    for (const event of events) {
        ...
        if (!Number.isFinite(occurredAtMs)) {
            throw new Error(
                'Session contains an invalid event date',
            );
        }

        const eventDurationMs = normalizeDurationMs(
            event.durationMs,
        );
        ...
        endedAtMs = Math.max(
            endedAtMs,
            occurredAtMs + eventDurationMs,
        );

        totalEngagedDurationMs = safeAdd(
            totalEngagedDurationMs,
            eventDurationMs,
        );
     ...
     */
    const engagedDurationMs = Math.min(
        totalEngagedDurationMs,
        durationMs,
    );
    ...
    const bounced =
        ...
        customEventCount === 0 &&
        engagedDurationMs <
        BOUNCE_ENGAGEMENT_THRESHOLD_MS;
...
function safeAdd(
    ...
    if (!Number.isSafeInteger(result)) {
        throw new RangeError(
            'Session duration exceeds Number.MAX_SAFE_INTEGER',
        );
    }
