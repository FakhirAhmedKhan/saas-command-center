...
export class WebsitesService {
    ...
    async create(
        ...
        const existing =
            await this.prisma.website.findUnique({
                where: {
                    workspaceId_domain: {
                        workspaceId,
                        domain,
        ...
        if (existing) {
            throw new ConflictException(
                'A website with this domain already exists in the workspace',
            );
        }
            ...
            return await this.prisma.$transaction(
                async (transaction) => {
                    ...
                    await this.writeActivity(
                        ...
                        {
                            activityType:
                                ApplicationActivityType.WEBSITE_CREATED,

                            title:
                                'Website created',

                            description:
                                `${created.name} was added to the website registry.`,
    ...
    async update(
        ...
        if (
            ...
            data.allowedOrigins = {
                set: this.normalizeAllowedOrigins(
                    undefined,
                    newDomain,
                ),
            };
            ...
            return await this.prisma.$transaction(
                async (transaction) => {
                    ...
                    await this.writeActivity(
                        ...
                        {
                            activityType:
                                ApplicationActivityType.WEBSITE_UPDATED,

                            title:
                                'Website updated',

                            description:
                                `${updated.name} configuration was updated.`,
        ...
        return this.prisma.$transaction(
            async (transaction) => {
                ...
                await this.writeActivity(
                    ...
                    {
                        activityType:
                            ApplicationActivityType.WEBSITE_ARCHIVED,

                        title:
                            'Website archived',

                        description:
                            `${updated.name} was archived and tracking was disabled.`,
                    },
                );
        ...
        return this.prisma.$transaction(
            async (transaction) => {
                ...
                await this.writeActivity(
                    ...
                    {
                        activityType:
                            ApplicationActivityType.WEBSITE_RESTORED,

                        title:
                            'Website restored',

                        description:
                            `${updated.name} was restored. Tracking remains disabled until enabled manually.`,
                    },
                );
        ...
        return this.prisma.$transaction(
            async (transaction) => {
                ...
                await this.writeActivity(
                    ...
                    {
                        activityType:
                            ApplicationActivityType.WEBSITE_TRACKING_KEY_ROTATED,

                        title:
                            'Tracking key rotated',

                        description:
                            `${updated.name} received a new tracking key.`,
        ...
        return this.prisma.$transaction(
            async (transaction) => {
                ...
                await this.writeActivity(
                    ...
                    {
                        activityType:
                            ApplicationActivityType.WEBSITE_CONNECTED,

                        title:
                            'Website connected',

                        description:
                            `${updated.name} was connected to ${application.name}.`,
        ...
        return this.prisma.$transaction(
            async (transaction) => {
                ...
                await this.writeActivity(
                    ...
                    {
                        activityType:
                            ApplicationActivityType.WEBSITE_DISCONNECTED,

                        title:
                            'Website disconnected',

                        description:
                            `${updated.name} was disconnected from its SaaS application.`,
    ...
    private async requireWebsite(
        ...
        const website =
            await client.website.findFirst({
                where: {
                    id: websiteId,
                    workspaceId,
                },
    ...
    private ensureNotArchived(
        ...
        if (website.archivedAt) {
            throw new ConflictException(
                'Restore the website before modifying it',
            );
        }
    }

    private generateTrackingKey():
        GeneratedTrackingKey {
        const prefix =
            randomBytes(8)
                .toString('hex');
    ...
    private normalizeAllowedOrigins(
        values:
            | string[]
            | undefined,
        domain: string,
        ...
        const origins =
            source.map((value) => {
                ...
                try {
                    ...
                    throw new BadRequestException(
                        `Allowed origin "${value}" is invalid`,
                    );
                }
