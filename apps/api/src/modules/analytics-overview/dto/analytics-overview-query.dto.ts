...
export class AnalyticsOverviewQueryDto {
    @ApiPropertyOptional({
        ...
        default:
            AnalyticsDatePreset.SEVEN_DAYS,
        description:
            'Analytics range calculated in the website timezone.',
    })
    @IsOptional()
    ...
    @ApiPropertyOptional({
        example: '2026-08-01',
        description:
            'Inclusive custom start date using YYYY-MM-DD.',
    })
    @ValidateIf(
        (
            query:
    ...
    @ApiPropertyOptional({
        example: '2026-08-07',
        description:
            'Inclusive custom end date using YYYY-MM-DD.',
    })
    @ValidateIf(
        (
            query:
