...
function asNumber(
  ...
  if (
    ...
    throw new Error(
      'Expected a numeric retention result',
    );
  }
  ...
  () => {
    ...
    afterAll(
      () => {
        ...
        if (
          previousEnvironment.normalized ===
          undefined
        ) {
    ...
    it(
      'deletes only processed raw events older than the raw cutoff',
      async () => {
        const owner =
        ...
        expect(
          asNumber(
            response.body.rawEventsDeleted,
          ),
        ).toBe(1);
    ...
    it(
      'deletes expired sessions and removes only orphaned visitors',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        expect(
          asNumber(
            response.body.sessionsDeleted,
          ),
        ).toBe(1);

        expect(
          asNumber(
            response.body.visitorsDeleted,
          ),
        ).toBe(1);
    ...
    it(
      'deletes expired hourly and daily aggregates while preserving recent buckets',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        expect(
          asNumber(
            response.body.hourlyAggregatesDeleted,
          ),
        ).toBe(1);

        expect(
          asNumber(
            response.body.dailyAggregatesDeleted,
          ),
        ).toBe(1);
    ...
    it(
      'limits retention changes to the requested website',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      ...
      async () => {
        ...
        expect(
          (
            await runAnonymousAnalyticsRetention(
              app,
              owner.workspaceId,
              website.id,
            )
