...
describe(
  ...
  () => {
    ...
    it(
      'rebuilds only the selected range and preserves events outside it',
      async () => {
        const owner =
        ...
        expect(
          outsideAfter.normalizedPath,
        ).toBe('/outside');
      },
    );

    it(
      'is idempotent across repeated reprocessing of the same range',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'rejects invalid and oversized date ranges',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'rejects invalid maxEvents and a range larger than maxEvents',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'allows OWNER and ADMIN but denies DEVELOPER and VIEWER',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'does not increment the processing total or move the normal cursor',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        const before =
          await prisma.analyticsProcessingState
            .findUniqueOrThrow({
              where: {
                websiteId:
                  website.id,
        ...
        expect(
          after.totalRawEventsProcessed,
        ).toBe(
          before.totalRawEventsProcessed,
        );

        expect(
          after.lastProcessedReceivedAt?.toISOString(),
        ).toBe(
    ...
    it(
      'rejects disabled and archived websites without deleting valid normalized data',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'requires authentication and hides foreign websites',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
