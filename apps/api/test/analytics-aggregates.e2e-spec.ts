...
describe(
  ...
  () => {
    ...
    it(
      'builds correct hourly and daily overview totals',
      async () => {
        const owner =
    ...
    it(
      'builds PAGE and CUSTOM_EVENT dimensions',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        await createRawAnalyticsEvent(
          ...
          website,
          {
            ...
            occurredAt,
            pageUrl:
              `${website.origin}/pricing/?utm_source=test&b=2&a=1`,
            pageTitle:
              'Pricing',
          },
        );
    ...
    it(
      'builds SOURCE, COUNTRY, DEVICE, BROWSER, and OPERATING_SYSTEM dimensions',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        await createRawAnalyticsEvent(
          ...
          {
            referrerUrl:
              'https://www.google.com/search?q=command+center',
            countryCode:
              'AE',
            userAgent,
          },
    ...
    it(
      'filters by period, dimension, date range, and limit',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        await prisma.analyticsHourlyAggregate
          .createMany({
            data:
              Array.from(
                {
    ...
    it(
      'rebuilds aggregates idempotently',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        const before =
          await prisma.analyticsDailyAggregate
            .findMany({
              where: {
                websiteId:
                  website.id,
    ...
    it(
      'allows VIEWER reads and denies outsider and anonymous reads',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        expect(
          (
            await getAnonymousAnalyticsAggregates(
              app,
              owner.workspaceId,
              website.id,
            )
    ...
    it(
      'rejects malformed aggregate queries and hides foreign websites',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'serializes BigInt duration without precision loss',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        expect(
          result.data[0]?.totalDurationMs,
        ).toBe(
          unsafeDuration.toString(),
        );
      },
    );
  },
