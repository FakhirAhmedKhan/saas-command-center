...
describe(
  'Analytics Engine Processing E2E',
  () => {
    let app:
      INestApplication;
    ...
    it(
      'processes pending raw events through the protected endpoint',
      async () => {
        const owner =
    ...
    it(
      'honors maxEvents and leaves the remaining raw events pending',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'creates a completed zero-work run when no events are pending',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'validates maxEvents boundaries',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'allows DEVELOPER processing but denies VIEWER processing',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'rejects processing for disabled and archived websites',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'marks the run and processing state as FAILED when a session ID is reused by another visitor',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        const run =
          await prisma.analyticsProcessingRun
            .findFirst({
              where: {
                websiteId:
                  trackedWebsite.id,
