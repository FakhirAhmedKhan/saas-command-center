...
/**
 * Phase 16 — Releases & Deployments E2E
 *
 ...
 *
 * Deployment state machine (deployment-transition.service.ts, TRANSITIONS map — this is the
...
describe('Phase 16 Releases & Deployments E2E', () => {
  ...
  it('creates a DRAFT release as OWNER when no scheduledAt is supplied', async () => {
    ...
    const persisted = await prisma.release.findUniqueOrThrow({ where: { id: created.id as string } });
  ...
  it('creates a SCHEDULED release when scheduledAt is supplied', async () => {
    ...
    expect(response.status).toBe(201);
    expect(body(response)).toMatchObject({ status: 'SCHEDULED', scheduledAt: '2026-09-01T00:00:00.000Z' });
  });
  ...
  it('allows the same version string across two DIFFERENT applications', async () => {
    ...
    const response = await request(app.getHttpServer())
      .post(
        `${API_PREFIX}/workspaces/${workspaceId}/applications/${otherApplication.id}/releases`,
      )
      .set(withBearer(ownerAccessToken))
      .send({ version: '2.0.0' });
  ...
  // ---------------------------------------------------------------------------------------
  // C. Deployment CRUD/state — belongs to release/environment/application
  // ---------------------------------------------------------------------------------------
  ...
  it('rejects a rollback whose target belongs to a different environment', async () => {
    ...
    await transition(ownerAccessToken, stagingDeploymentId, { status: DeploymentStatus.IN_PROGRESS });
    await transition(ownerAccessToken, stagingDeploymentId, { status: DeploymentStatus.SUCCESSFUL });
  ...
  it('rejects a rollback whose target is not SUCCESSFUL', async () => {
    ...
    await transition(ownerAccessToken, activeDeploymentId, { status: DeploymentStatus.IN_PROGRESS });
  ...
  // ---------------------------------------------------------------------------------------
  // H. Data integrity — optimistic concurrency
  // ---------------------------------------------------------------------------------------
