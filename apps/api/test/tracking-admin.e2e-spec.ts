...
describe(
  ...
  () => {
    ...
    it(
      'returns empty tracking status before the first event',
      async () => {
        const owner =
    ...
    it(
      'returns grouped counts and at most five recent events',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
        ...
        expect(
          status.counts.PAGE_VIEW,
        ).toBe(2);

        expect(
          status.counts.HEARTBEAT,
        ).toBe(1);
    ...
    it(
      'allows VIEWER to inspect tracking status',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'prevents outsider and anonymous tracking-admin access',
      async () => {
        const owner =
          await registerWorkspaceTestUser(
            app,
    ...
    it(
      'rejects malformed, unknown, and foreign website IDs',
      async () => {
        const alphaOwner =
          await registerWorkspaceTestUser(
            app,
