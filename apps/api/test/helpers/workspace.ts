...
export async function registerWorkspaceTestUser(
    ...
    if (!databaseUser) {
        throw new Error(
            `Registered user ${input.email} was not found in the database`,
        );
    }
    ...
    if (!ownerMembership) {
        throw new Error(
            `Initial OWNER membership was not created for ${input.email}`,
        );
    }
...
export function expectBusinessRuleRejected(
    response: Response,
): void {
    expect([
        400,
