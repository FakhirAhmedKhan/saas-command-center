...
export function enumValue<T extends string>(enumObject: StringEnumObject<T>, index = 0): T {
  ...
  if (!value) {
    throw new Error('Generated enum has no values');
  }
...
export function expectMutationSuccess(response: Response): void {
...
export async function createApplication(
  ...
  return {
    id: readEntityId(response, ['application']),

    payload,

    record: readApiRecord(response, ['application']),
