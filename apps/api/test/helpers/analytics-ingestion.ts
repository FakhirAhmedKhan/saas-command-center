...
export function uniqueTrackerId(
  prefix = 'tracker',
): string {
  return `${prefix}_${Date.now()}_${randomPart()}`;
}

export function uniqueOrigin(
  prefix = 'analytics',
): string {
...
export function readCollectResult(
  response: Response,
): CollectResponseBody {
  const body =
    asRecord(
  ...
  if (
    !body ||
    typeof body.accepted !==
    'number' ||
    typeof body.duplicates !==
    'number' ||
    typeof body.receivedAt !==
    'string'
  ) {
    throw new Error(
      [
        'Unexpected collection response.',
...
export async function getTrackingStatus(
  ...
  return actor.agent
    .get(
      analyticsIngestionRoutes.status(
        actor.workspaceId,
        websiteId,
      ),
    )
...
export async function listRawEvents(
  ...
  return actor.agent
    .get(
      analyticsIngestionRoutes.events(
        actor.workspaceId,
        websiteId,
      ),
    )
...
export function readTrackingStatus(
  response: Response,
): TrackingStatusBody {
  const body =
    asRecord(
  ...
  if (
    ...
    !website ||
    !counts ||
    typeof body.connected !==
    'boolean' ||
    typeof body.totalEvents !==
    'number'
  ) {
    throw new Error(
      [
        'Unexpected tracking status response.',
        `Received: ${JSON.stringify(
          response.body,
        )}`,
      ].join(' '),
...
export function readRawEventList(
  response: Response,
): RawEventListBody {
  const body =
    asRecord(
  ...
  if (
    ...
    typeof meta.total !==
    'number' ||
    typeof meta.totalPages !==
    'number' ||
    typeof meta.hasNextPage !==
    'boolean' ||
    typeof meta.hasPreviousPage !==
    'boolean'
  ) {
    throw new Error(
      [
        'Unexpected raw-event list response.',
        `Received: ${JSON.stringify(
          response.body,
        )}`,
      ].join(' '),
...
export function expectCollectionAccepted(
  ...
  if (response.status !== 202) {
    throw new Error(
      `Collect failed: ${response.status}. Body: ${JSON.stringify(response.body)}`,
    );
  }
