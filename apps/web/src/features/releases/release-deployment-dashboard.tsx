...
export function ReleaseDeploymentDashboard({
  workspaceId,
  applicationId,
}: DashboardProps) {
  const [
  ...
  async function changeStatus(
    ...
    if (
      ...
      const reason =
        window.prompt(
          'Enter the deployment failure reason.',
        );
    ...
    if (
      ...
      if (
        ...
        setActionError(
          'No earlier successful deployment exists for this environment.',
        );
        ...
        candidates
          .map(
            (
              ...
              `${index + 1}. ${candidate.release.version}`,
          )
          .join('\n');
    ...
    const confirmed =
      window.confirm(
        `Change deployment from ${deployment.status} to ${target}?`,
      );
