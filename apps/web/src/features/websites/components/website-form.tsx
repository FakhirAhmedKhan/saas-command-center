...
interface WebsiteFormProps {
  ...
  onSubmit: (payload: CreateWebsitePayload) => Promise<void>;
}
...
export function WebsiteForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (name.trim().length < 2) {
      setError('Website name must contain at least two characters.');

      return;
    }

    if (!domain.trim()) {
      setError('Website domain is required.');
    ...
    try {
      ...
      setError(getWebsiteError(submitError, 'Unable to save website.'));
    } finally {
      setSubmitting(false);
    }
  }
