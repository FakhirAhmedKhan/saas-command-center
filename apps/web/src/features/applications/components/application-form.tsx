...
export function ApplicationForm({
    ...
    const [category, setCategory] =
        useState<ApplicationCategory>(
            application?.category ?? 'SAAS',
        );

    const [status, setStatus] =
        useState<ApplicationStatus>(
            application?.status ?? 'IDEA',
        );

    const [priority, setPriority] =
        useState<ApplicationPriority>(
    ...
    const [
        ...
        toDateInputValue(
            application?.targetLaunchAt,
        ),
    );

    const [launchedAt, setLaunchedAt] =
        useState(
            toDateInputValue(
                application?.launchedAt,
            ),
        );
    ...
    async function handleSubmit(
        ...
        try {
            await onSubmit({
                ...
                priority,
                startedAt:
                    toApiDateValue(startedAt),
                targetLaunchAt:
                    toApiDateValue(targetLaunchAt),
                launchedAt:
                    toApiDateValue(launchedAt),
            });
            ...
            <Card>
                <CardHeader>
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-500">
                        Add the main information used to
                        organize and monitor this SaaS
                        application.
                    </p>
                </CardHeader>
