...
export const metadata: Metadata = {
  ...
  description: 'Connect repositories, applications, deployments, monitoring, analytics, infrastructure, and teams inside one centralized SaaS operations workspace.',
};
      ...
      <div className='mx-auto flex h-[72px] max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8'>
        ...
        <nav className='hidden items-center gap-1 rounded-full border border-white/[0.06] bg-white/[0.025] p-1 md:flex'>
          {(
            ...
            <Link key={href} href={href} className='rounded-full px-4 py-2 text-[13px] font-medium text-zinc-400 transition hover:bg-white/[0.05] hover:text-white'>
              {label}
            </Link>
          ))}
        </nav>
        ...
        <div className='flex items-center gap-2'>
          ...
          <Link href='/register' className='hidden items-center gap-2 rounded-full bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-black transition hover:bg-emerald-600 sm:flex'>
            Get started
          ...
          <aside className='hidden border-r border-white/[0.06] bg-black/20 p-4 lg:block'>
            ...
            <div className='space-y-1'>
              {[
                ...
                return (
                  <div key={label as string} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-xs ${index === 0 ? 'border border-white/[0.06] bg-white/[0.055] text-white' : 'text-zinc-500'}`}>
                ...
                <div className='space-y-4'>
                  {[
                    ...
                    <div key={title} className='flex items-start gap-3'>
                      <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${tone === 'blue' ? 'bg-blue-400' : tone === 'violet' ? 'bg-violet-400' : 'bg-emerald-400'}`} />
      ...
      <div className='relative mx-auto max-w-7xl'>
        <div className='mx-auto max-w-4xl text-center'>
          ...
          <p className='mx-auto mt-7 max-w-2xl text-pretty text-base leading-7 text-zinc-400 sm:text-lg'>
            Bring repositories, applications, deployments, analytics, infrastructure health, monitoring, and team operations into one command center built for developers.
          </p>

          <div className='mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row'>
            <Link href='/register' className='group flex h-12 w-full items-center justify-center gap-2 rounded-full bg-emerald-500 px-6 text-sm font-semibold text-black transition hover:bg-emerald-600 sm:w-auto'>
  ...
  return (
    <section id='integrations' className='border-y border-white/[0.055] bg-white/[0.012] px-5 py-9 sm:px-6 lg:px-8'>
      <div className='mx-auto max-w-7xl'>
        <div className='mb-7 text-center text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-700'>Connect the infrastructure your product already runs on</div>
...
function FeatureCard({ eyebrow, title, description, icon, children, className = '' }: { eyebrow: string; title: string; description: string; icon: React.ReactNode; children?: React.ReactNode; className?: string }) {
  return (
    <article className={`group relative overflow-hidden rounded-[22px] border border-white/[0.07] bg-[#090b10] p-6 transition duration-500 hover:border-white/[0.13] sm:p-7 ${className}`}>
        ...
        <div className='grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end'>
          ...
          <div className='max-w-xl lg:justify-self-end'>
            <p className='text-base leading-7 text-zinc-500'>
              SaaS Command Center turns scattered operational data into one structured workspaceâ€”giving your team a single place to understand what is running, what changed, and what needs attention.
            </p>
          </div>
        </div>
            ...
            <div className='absolute inset-x-6 bottom-6'>
              <div className='flex h-24 items-end gap-2'>
                {[39, 57, 44, 72, 61, 88, 69, 94].map((height, index) => (
                  <div key={`${height}-${index}`} className='flex-1 rounded-t-sm bg-gradient-to-t from-blue-600/15 to-blue-400/55' style={{ height: `${height}%` }} />
                ))}
              </div>
    ...
    <section id='workflow' className='border-y border-white/[0.055] bg-white/[0.012] px-5 py-24 sm:px-6 sm:py-32 lg:px-8'>
      <div className='mx-auto max-w-7xl'>
        <div className='mx-auto max-w-2xl text-center'>
          ...
          <p className='mt-5 text-base leading-7 text-zinc-500'>Command Center is designed around the way software teams already workâ€”not around another complicated setup process.</p>
        </div>
          ...
          {steps.map((step) => (
            <article key={step.number} className='relative rounded-[20px] border border-white/[0.065] bg-[#090b10] p-6'>
              <div className='relative z-10 mb-10 flex items-center justify-between'>
                <div className='flex h-11 w-11 items-center justify-center rounded-xl border border-blue-400/15 bg-blue-400/[0.06] text-blue-300'>{step.icon}</div>
    ...
    <section className='px-5 py-24 sm:px-6 sm:py-32 lg:px-8'>
      <div className='mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-2'>
        <div>
          ...
          <p className='mt-6 max-w-xl text-base leading-7 text-zinc-500'>Every workflow is centered around repositories, environments, application state, deployments, observability, and secure team collaboration.</p>
    ...
    <section className='border-y border-white/[0.055] bg-white/[0.012] px-5 py-24 sm:px-6 sm:py-32 lg:px-8'>
      <div className='mx-auto max-w-7xl'>
        <div className='mx-auto max-w-2xl text-center'>
          ...
          <p className='mt-5 text-base leading-7 text-zinc-500'>Connect the systems your team already uses without forcing your product into a new deployment model.</p>
        </div>
          ...
          <div className='mx-auto max-w-3xl'>
            ...
            <div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-4'>
              {[
                ...
                return (
                  <div key={title as string} className='flex items-center justify-center gap-2 rounded-xl border border-white/[0.055] bg-black/30 px-4 py-3 text-[10px] text-zinc-500'>
            ...
            <div className='relative border-b border-white/[0.06] p-8 sm:p-10 lg:border-b-0 lg:border-r lg:p-14'>
              ...
              <div className='relative'>
                ...
                <p className='mt-5 max-w-lg text-sm leading-7 text-zinc-500'>
                  Authentication, workspace isolation, role checks, secure cookies, refresh-token rotation, webhook verification, rate limiting, and production configuration validation are first-class parts of the
                  platform.
                </p>
              </div>
            </div>
...
function FAQ() {
  const questions = [
    ...
    ['Can I manage multiple SaaS products?', 'Yes. Workspaces are designed to organize multiple applications, repositories, websites, environments, integrations, and team members.'],
    ['Does it connect directly to GitHub?', 'Yes. The platform includes GitHub App based repository connectivity, installation authorization, repository importing, code exploration, and repository analysis workflows.'],
    [
    ...
    <section className='px-5 py-24 sm:px-6 sm:py-32 lg:px-8'>
      <div className='mx-auto grid max-w-7xl gap-14 lg:grid-cols-[0.7fr_1.3fr]'>
        <div>
          ...
          <p className='mt-5 max-w-sm text-sm leading-6 text-zinc-500'>SaaS Command Center is designed as an operational layer around your existing engineering stack.</p>
        </div>
    ...
    <section className='px-5 pb-24 sm:px-6 sm:pb-32 lg:px-8'>
      <div className='relative mx-auto max-w-7xl overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#0a0c12] px-6 py-20 text-center sm:px-10 sm:py-24'>
        ...
        <div className='relative mx-auto max-w-3xl'>
          ...
          <p className='mx-auto mt-6 max-w-xl text-base leading-7 text-zinc-500'>Create one workspace for the code, services, deployments, analytics, and people behind your SaaS.</p>
          ...
          <div className='mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row'>
            ...
            <Link href='/login' className='flex h-12 w-full items-center justify-center rounded-full border border-white/10 bg-black/20 px-6 text-sm font-medium text-white transition hover:bg-white/[0.05] sm:w-auto'>
              Sign in
            </Link>
          </div>
        </div>
