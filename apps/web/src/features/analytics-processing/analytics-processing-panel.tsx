...
import {
    useAnalyticsProcessing,
} from './use-analytics-processing';
...
export function AnalyticsProcessingPanel({
    ...
    async function reprocess():
        ...
        try {
            ...
            await queueAnalyticsReprocessing(
                workspaceId,
                websiteId,
                {
                    from:
            ...
            setSuccessMessage(
                'Analytics reprocessing was queued.',
            );
    ...
    async function retryRun(
        ...
        const confirmed =
            window.confirm(
                'Retry this failed analytics processing run?',
            );
        ...
        try {
            await retryAnalyticsProcessingRun(
                workspaceId,
                websiteId,
                runId,
            );
            ...
            setSuccessMessage(
                'Failed processing run was queued for retry.',
            );
        ...
        <main className="space-y-6">
            <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-sm text-slate-600">
                    View processing health,
                    queued work, retries and
                    failed batches.
                </p>
            </header>
            ...
            {data.canReprocess ? (
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                        Rebuild a limited analytics
                        date range. Existing valid
                        data is preserved when the
                        rebuild fails.
                    </p>
