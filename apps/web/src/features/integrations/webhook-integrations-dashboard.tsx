          ...
          {catalog.map(
            (event) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={form.eventTypes.includes(
                    event.type,
                  )}
                  onChange={() => {
                    toggleEvent(
...
export function WebhookIntegrationsDashboard({
  workspaceId,
}: WebhookIntegrationsDashboardProps) {
  const [
    data,
    setData,
  ] =
    useState<
      WebhookListResponse | null
    >(null);
      ...
      {secret ? (
        <section className="rounded-2xl border border-amber-300 bg-amber-50 p-5">
          ...
          <p className="mt-1 text-sm text-amber-800">
            This secret is shown only
            once. Store it securely before
