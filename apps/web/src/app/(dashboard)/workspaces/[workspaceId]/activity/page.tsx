...
import type { WorkspaceActivity } from '@/features/activity/workspace-activity-api';
...
export default function WorkspaceActivityPage() {
    ...
    const timer = window.setTimeout(
      () => {
        ...
        void getWorkspaceActivities(
          ...
          .then((response: { items: SetStateAction<WorkspaceActivity[]> }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-4 sm:p-6 lg:p-8">
      <header>
        <h1 className="text-2xl font-bold text-slate-950">Workspace activity</h1>

        <p className="mt-1 text-sm text-slate-600">Search who changed what and when.</p>
      </header>
      ...
      <section className="mt-5 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-4">
        ...
        <label>
          <span className="text-xs font-medium uppercase text-slate-500">Resource</span>
      ...
      {loading ? (
        ...
        <section className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {activities.map((activity) => (
            <article key={activity.id} className="border-b border-slate-100 p-5 last:border-0">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-semibold text-slate-950">{humanize(activity.activityType)}</p>
