...
test.describe('Batch 11 real analytics pipeline', () => {
  ...
  test.beforeAll(async ({
    ...
    visitorId =
      trackerIdentifier('visitor');
    sessionId =
      trackerIdentifier('session');
  ...
  test('accepts real tracker events through the public collect endpoint', async ({
    request,
  }) => {
    const timestamp =
    ...
    const response =
      await request.post(
        `${state.apiUrl}/collect`,
        {
          headers: {
            Origin:
              state.trackingOrigin,
            'User-Agent':
              'Batch11FullStack/1.0 Chrome',
            'Content-Type':
              'text/plain',
          },
          data: JSON.stringify({
            websiteId:
              state.baselineWebsite.id,
            trackingKey:
              state.baselineWebsite.trackingKey,
            sdkVersion:
              '1.0.0-batch11',
            sentAt: timestamp,
            events: [
              {
                eventId:
                  trackerIdentifier('page'),
                type: 'PAGE_VIEW',
                visitorId,
                sessionId,
                timestamp,
                url:
                  `${state.trackingOrigin}/pricing`,
                title:
                  'Batch 11 Pricing',
                referrer:
  ...
  test('processes the events and displays normalized analytics', async () => {
    ...
    expect(
      (await processingResponse).status(),
    ).toBe(201);
